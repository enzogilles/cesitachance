  ...
  fetch(`${BASE_URL}wishlist/add`, {
    ...
    .then(data => {
      if (data.success) {
        ...
        // Removed the setTimeout redirect that was here
      } else {
...
// Mise à jour de la fonction pour voir la wishlist d'un étudiant spécifique
document.querySelectorAll('.view-student-wishlist').forEach(button => {
  button.addEventListener('click', function() {
    const studentId = this.getAttribute('data-student-id');
    window.location.href = `${BASE_URL}wishlist/view/${studentId}`;
  });
});
